...
| Path | Purpose |
|------|---------|
| `ios/Raide.xcodeproj` | Xcode project (iOS 17+, SwiftUI + UIKit bridges) |
| `ios/Raide/` | App sources, mock trip layer, map shell, App Icon, design system |
...
| Google Maps | SPM (Raide demo maps) |
...
1. Open `ios/Raide.xcodeproj` (Mac) or push to Codemagic
2. Resolve SPM (ComponentsKit + Google Maps)
3. Optionally set `RAIDE_GOOGLE_MAPS_API_KEY`
4. Run: Home → destination → ride tier → searching → assigned → trip → complete

## Temporary

`Wells-Gas-Level-Monitor-main/` is a reference copy used only for scaffolding. Delete it once you’re satisfied the Raide base is complete.
