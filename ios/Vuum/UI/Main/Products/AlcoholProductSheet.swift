...
/// Age-gated alcohol delivery sheet — local catalog into courier booking.
struct AlcoholProductSheet: View {
    ...
    @State private var selectedItems: Set<String> = ["beer"]
    ...
    private var estimate: RideTier {
        ProductCatalogTiers.alcohol(distanceMeters: meters)
    }
    ...
    private var composedNotes: String {
        let items = catalog
            ...
            .map(\.title)
        var parts: [String] = ["Age verified at door"]
        if !items.isEmpty {
    ...
    var body: some View {
        NavigationStack {
            ProductBookingForm(
                title: L10n.t("services.alcohol"),
                subtitle: "Licensed shops · ID check on delivery.",
                symbol: "wineglass.fill",
                confirmTitle: "Continue to book",
                pickup: $pickup,
                dropoff: $dropoff,
                estimate: estimate,
                canConfirm: ageConfirmed && !selectedItems.isEmpty
            ) {
                Section("Age confirmation") {
                    Toggle("I confirm I am of legal drinking age", isOn: $ageConfirmed)
                }
                Section("Order") {
            ...
            .navigationTitle(L10n.t("services.alcohol"))
