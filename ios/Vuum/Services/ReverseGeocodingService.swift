...
enum ReverseGeocodingService {
    ...
    static func coordinateFallback(_ location: CLLocation) -> AddressLabel {
        ...
        return AddressLabel(
            ...
            subtitle: String(format: "%.5f, %.5f", lat, lng)
        )
    }
    ...
    private static func label(from result: GoogleGeocodeResult) -> AddressLabel {
        ...
        if let streetLine, !streetLine.isEmpty {
            ...
            name = formatted.trimmingCharacters(in: .whitespacesAndNewlines)
        } else {
            name = "Current location"
        }

        let subtitleParts = [locality, admin].compactMap { $0 }.filter { !$0.isEmpty }
        let subtitle: String
        if !subtitleParts.isEmpty {
            subtitle = subtitleParts.joined(separator: ", ")
        } else if let formatted = result.formattedAddress, formatted != name {
            let rest = formatted
                .split(separator: ",")
                .dropFirst()
                .map { $0.trimmingCharacters(in: .whitespacesAndNewlines) }
                .filter { !$0.isEmpty }
            subtitle = rest.isEmpty ? "Current location" : rest.prefix(2).joined(separator: ", ")
        } else {
    ...
    private static func appleReverseGeocode(_ location: CLLocation) async -> AddressLabel? {
        await withCheckedContinuation { continuation in
            CLGeocoder().reverseGeocodeLocation(location) { placemarks, error in
                ...
                continuation.resume(returning: label(from: place))
            }
        }
    }

    private static func label(from placemark: CLPlacemark) -> AddressLabel {
        ...
        let name = (streetLine?.trimmingCharacters(in: .whitespacesAndNewlines)).flatMap {
            $0.isEmpty ? nil : $0
        } ?? "Current location"
        ...
        let subtitle = subtitleParts.isEmpty
            ? "Current location"
            : Array(subtitleParts.prefix(2)).joined(separator: ", ")
